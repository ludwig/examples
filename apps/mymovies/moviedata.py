...
class MovieContainer(object):
    ...
    and year, so if either of these fields is changed the movies must
    be re-sorted. For this reason (and to maintain the dirty flag), all
    ...
    def load(self, fname=QString()):
        if not fname.isEmpty():
            self.__fname = fname
        if self.__fname.endsWith(".mqb"):
            self.loadQDataStream()
        elif self.__fname.endsWith(".mpb"):
            self.loadPickle()
        elif self.__fname.endsWith(".mqt"):
            self.loadQTextStream()
        elif self.__fname.endsWith(".mpt"):
            self.loadText()
    ...
    def saveQDataStream(self):
        ...
        try:
            ...
            return True, "Saved %d movie records to %s" % \
                    (len(self.__movies), QFileInfo(self.__fname).fileName())
...
class SaxMovieHandler(QXmlDefaultHandler):
    ...
    def startElement(self, namespaceURI, localName, qName, attributes):
        if qName == "MOVIE":
            ...
            self.minutes = intFromQStr(attribute.value("MINUTES"))
    ...
    def endElement(self, namespaceURI, localName, qName):
        if qName == "MOVIE":
            ...
            self.movies.add(Movie(self.title, self.year, self.minutes,
                                  self.acquired, decodedNewLines(self.notes)))
            self.clear()
        elif qName == "TITLE":
