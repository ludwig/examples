/* From /usr/share/doc/blitz++/examples/cfd.cpp
 *
 * This is a small 4th-order Computational Fluid Dynamics solver.
 * It illustrates multicomponent arrays (i.e. vector fields) and
 * stencil objects. It is implemented in "C++Tran" style for
 * simplicity.
 *
 * Incompressible flow is assumed.
 *
 * The simulation is of water in a closed cube.  The water is
 * initially quiescent.  At a single layer near the bottom of
 * the cube, a forcing function simulates the effect of a
 * (highly idealized) propeller.
 *
...
/*
 * This is a "work in progress".  Things I am unhappy about:
 *   - arrays, geometry, and boundary conditions are separate
 *     objects. This results in a certain amount of kludginess.
 *     For example, the stencil operators take geometry as a
 *     2nd parameter. On the plus side, one geometry object is
 *     shared among many arrays.
 ...
 *     be nice if stencil operators could be "expression templetized"
 *     so that these equations could be moved to the appropriate place
 *     in the code.  This probably wouldn't be too much work.
 *   - Stencil objects can't take scalar arguments, so all the
 *     scalars have to be globals.  Big ugh.  There is a fix for
 *     this, but it hasn't been implemented yet.
 *   - There are serious bugs, this simulation doesn't work
 *     very well.
 */
 ...
 * to be placed in the global scope. Ugh. This restriction will be removed
 * eventually.
 */
...
/*
 * The "geometry" object specifies how an array is mapped into real-world space.
 * In this case, "UniformCubicGeometry" is used, which means that the real-world
 * grid is orthogonal, regularly spaced, with the same spatial step in each
 * dimension.
 */
           ...
           scalarField& P_rhs, vectorField& advect, vectorField& force)
{
    ...
    for (int i = force.lbound(firstDim); i <= force.ubound(firstDim); ++i)
    {
        ...
        {
            // Vector from the centre of the stirring to the current coordinate
            ...
            if (norm(r) < stirRadius)
            {
                ...
                // coordinate yields the direction of the force. Multiply by
                // gravity to get a reasonable magnitude (max force = 5 * gravity)
                force(i,j,centrez) += cross(zaxis,r) * (5 * gravity_z / stirRadius);
            }
        }
    }
#endif // NO_STIRRING
             ...
             scalarField& P_rhs, vectorField& advect, vectorField& force)
{
    ...
    conjugateGradientSolver(P_solver_update(), P, P_rhs, 1e-8, PressureBCs(gravityPressureGradient));
...
int main()
{
    ...
    for (int i = 0; i < nIters; ++i)
    {
        ...
        // Update the current time, turn off the force when we pass forceTurnoff
