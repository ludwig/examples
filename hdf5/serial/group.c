...
/*
 * This program creates a group in the file and two datasets in that group.
 * 
 * Then, a hard link to the group object is created, and one of the datasets
 * is accessed under the new name.
 *
 * Iterator functions are used to find information about the objects in the
 * root group and in the created group.
 */
...
herr_t file_info(hid_t loc_id, const char *name, void *opdata)
{
    ...
    /*
     * Display group name. The name is passed to the function
     * by the Library. Some magic :-)
     */
    printf("\n");
    printf("Name : ");
    puts(name);
...
int main(void)
{
     ...
     * The dataset creation property list is modified to use GZIP compression
     * with the compression effort set to 6. Note that compression can be used
     * only when a dataset is chunked.
     */
    ...
    dataset = H5Dcreate(file, "/Data/Compressed_Data", H5T_NATIVE_INT, dataspace, plist);
    ...
    dataset = H5Dcreate(file, "/Data/Float_Data", H5T_NATIVE_FLOAT, dataspace, H5P_DEFAULT);
    ...
    if (dataset < 0)
    {
        printf(" Dataset 'Compressed_Data' was not found!\n");
        exit(1);
    }
    ...
    /*
     * We can access "Compressed_Data" dataset using the newly
     * created hard link "Data_new".
     */
    dataset = H5Dopen(file, "/Data_new/Compressed_Data");
    if (dataset < 0)
    {
        printf(" Dataset '/Data_new/Compressed_Data' not found!\n");
        exit(1);
    }
     ...
     * of the objects in the file root directory.
     */
    ...
    /*
     * Use iterator to see names of the objects in the group /Data_new
     */
    idx_g = H5Giterate(file, "/Data_new", NULL, group_info, NULL);
